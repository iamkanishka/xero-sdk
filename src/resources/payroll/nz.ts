...
/** Payroll NZ (New Zealand) API client. */
export class PayrollNzClient extends PayrollClientBase {
  ...
  async getEmployeeLeave(
    ...
    const env = await this.g<{ LeaveLines: XeroObject[] }>(
      token,
      tenantId,
      `/Employees/${id}/leave`,
      toQuery(params),
  ...
  // -- Pay Run Calendars, Payslips, Earnings Rates, Deduction Types --------
  ...
  // -- Leave Types, Reimbursements, Timesheets, Settings -------------------
  ...
  getReimbursement(
    ...
    return this.g(token, tenantId, `/Reimbursements/${id}`, undefined, options);
  }
  createReimbursement(
    token: XeroToken,
    tenantId: string,
